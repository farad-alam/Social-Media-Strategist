...
export default async function IndustryPageTemplate({ industry }: { industry: string }) {
  const data = await getIndustryPage(industry);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-12 gap-12 lg:gap-16 items-start">
            ...
            <div className="lg:col-span-8 space-y-12">
              {data.articleSections && data.articleSections.map((section, idx) => (
              ...
              {/* Linked blog post CTA */}
              {data.linkedBlogPost && (
                <ScrollReveal>
                  <div className="rounded-2xl border border-primary-100 bg-primary-50 p-8">
                    <p className="text-sm font-semibold uppercase tracking-widest text-primary mb-2">Related Article</p>
                    <h3 className="text-xl font-bold text-slate-900 mb-4">{data.linkedBlogPost.title}</h3>
                    <Button href={`/blog/${data.linkedBlogPost.slug}`} variant="primary" size="sm">
            ...
            <div className="lg:col-span-4 sticky top-32 space-y-8">
               <div className="bg-primary-50 rounded-2xl p-8 border border-primary-100">
                 ...
                 <ul className="space-y-4">
                   {data.challenges && data.challenges.map((challenge, i) => (
               ...
               <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-lg">
                 ...
                 <ul className="space-y-4">
                   {data.strategy && data.strategy.map((item, i) => (
                     <li key={i} className="flex items-start gap-3 text-slate-700">
